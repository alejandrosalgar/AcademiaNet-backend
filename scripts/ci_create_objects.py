...
def initialize_functions():
    global CONFIG_FILES
    # submodules = [f"src/{f}" for f in os.listdir("src")]
    # submodules.remove("src/neojumpstart_core_backend")
    # submodules_config = [f"{path}/configuration.json" for path in submodules]
    # submodules_config = ["src/neojumpstart_core_backend/configuration.json"] + submodules_config
...
def create_components(credential):
    for config_file in CONFIG_FILES:
        ...
        for component in components:
            ...
            """
            count = sql_context.exec(sql, {}, **credential["rds_params"])[0]["count"]
            if not count:
                execute = True
        ...
        if execute:
            sql_context.exec(components_sql, {}, **credential["rds_params"])
...
def create_object_limit(credential):
    ...
    has_columns = sql_context.exec(sql, {}, **credential["rds_params"])
    ...
    for config_file in CONFIG_FILES:
        ...
        if len(has_columns) == 0:
            ...
            if execute:
                sql_context.exec(objects_sql, {}, **credential["rds_params"])
        # Objects master with events
            ...
            for obj in objects:
                ...
                WHERE table_name = '{obj[0]}' AND tenant_id = '{credential['tenant_id']}'
                """
                object_data = sql_context.exec(sql, {}, **credential["rds_params"])
                ...
                if obj[5] is not None:
                    ...
                    parent_object_data = sql_context.exec(
                        parent_object_sql, {}, **credential["rds_params"]
                    )
                    parent_object_id = (
                        parent_object_data[0]["object_id"] if len(parent_object_data) else "null"
                    )
...
def add_relation_objects_components(credential):
    for config_file in CONFIG_FILES:
        ...
        for object in objects:
            ...
            """
            id_component = sql_context.exec(sql, {}, **credential["rds_params"])[0]["components_id"]
